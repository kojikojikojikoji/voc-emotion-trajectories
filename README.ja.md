...
3 冊とも、ダウンロードなしで `data/sample/` の 1,000 会話サンプルにフォールバックして動く（CI もこの経路）。コミット済みの出力はダウンロードスクリプトで取得した全 13,118 会話（約 4.5 MB）での実行結果で、各ノートブックの実行時間は 1 分未満。
...
- データ利用時の引用: Li, Su, Shen, Li, Cao, Niu, "DailyDialog: A Manually Labelled Multi-turn Dialogue Dataset" (IJCNLP 2017)。
- CC BY-NC-SA 4.0 は非商用ライセンス。コードは MIT だが、データを使った成果物にはデータ側のライセンスが及ぶ。
...
src/voc_arc/          データ読み込み・分類器・軌跡レイヤー・作図
tests/                pytest（遷移行列の手計算突合、埋め込みシフト検出を含む）
scripts/download_data.py
scripts/make_sample.py
data/sample/          出典明記付きの 1,000 会話サンプル
assets/               上掲の代表図
```
...
- 単一データセット・単一分割・単一分類器構成。Transformer との比較は意図的に含めない（本リポジトリはその比較の基準点となる軽量リファレンス）。
- 英語のみ。文字 n-gram 特徴はそのままでは他言語に転用できない。

## ライセンス

コードは MIT（[LICENSE](./LICENSE)）。DailyDialog データ（コミット済みサンプルを含む）は CC BY-NC-SA 4.0（上表参照）。
